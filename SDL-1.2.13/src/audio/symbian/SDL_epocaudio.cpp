...
void CEpocAudio::SetVolume(TInt aVolume)
{
    if(iPlayer != NULL)
    {
...
// New functions for volume control.
extern "C"
{

void EPOC_SetAudioVolume(int volume)
{
    SDL_AudioDevice* ad = current_audio;
    if(ad != NULL)
    {
	CEpocAudio::Current(ad).SetVolume(volume);
    }	
}
   
int EPOC_GetAudioVolume()
{
    SDL_AudioDevice* ad = current_audio;
    if(ad != NULL)
    {
	return CEpocAudio::Current(ad).GetVolume();
    }
    return -1;
}

const int EPOC_GetAudioMaxVolume()
{
    return 256;
}
